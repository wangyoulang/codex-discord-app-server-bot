...
def test_worker_collects_inline_markdown_link_artifacts_and_keeps_surrounding_text() -> None:
    class FakeClient:
        def thread_start(self, params: dict) -> dict:
            assert params == {"cwd": "/repo"}
            return {"thread": {"id": "thr_1"}}
                            ...
                            "items": [
                                {
                                    "id": "item_1",
                                    "type": "agentMessage",
                                    "text": "文件在 [screen.png](/repo/artifacts/screen.png)，请确认",
                                }
                            ],
                        }
                    ]
    ...
    async def scenario() -> None:
        ...
        assert result.final_text == "文件在，请确认"
        assert len(result.assistant_messages) == 1
        assert result.assistant_messages[0].item_id == "item_1"
        assert result.assistant_messages[0].text == "文件在，请确认"
        assert result.image_artifacts == [
            OutputImageArtifact(
                item_id="item_1:media:0",
                path="/repo/artifacts/screen.png",
